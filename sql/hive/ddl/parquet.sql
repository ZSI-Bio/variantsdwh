...
CREATE TABLE `dim_genomic_position_ensembl_pq`(
   `gp_id` bigint,
   `gp_chr` string,
   `gp_start` int,
   `gp_stop` int,
...
CREATE TABLE `dim_genomic_position_refseq_pq`(
   `gp_id` bigint,
   `gp_chr` string,
   `gp_start` int,
   `gp_stop` int,
...
CREATE TABLE `dim_disease_pq`(
   `d_id` bigint,
   `d_omim_id` string,
   `d_disease` string)
 STORED AS PARQUET;

CREATE TABLE `dim_variant_predictions_pq`(
   `v_id` bigint,
   `v_chr` string,
   `v_pos` int,
   `v_alt` string,
...
CREATE TABLE fact_agg_genotypes_counts_pq STORED AS PARQUET AS
SELECT
g.f_ensembl_gp_id    ,
g.f_refseq_gp_id     ,
g.f_v_id             ,
...
FROM   (SELECT f_geo_id,
  ...
  CREATE TABLE fact_agg_genotypes_counts_dims_pq STORED AS PARQUET AS
  SELECT
  f_ensembl_gp_id            ,
  f_refseq_gp_id             ,
  f_v_id                     ,
