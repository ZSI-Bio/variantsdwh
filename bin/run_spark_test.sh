...
SPARK_PARAMS="--master yarn-client --executor-memory 4g --num-executors 160 --driver-memory 4g  --hiveconf hive.server2.thrift.port=12000"
...
sleep 30
#spark2
./run_queries.sh --dbname=${DB_NAME} --iter=${ITER}  --engine=spark2 --dryRun=no --path=${PARENT_QUERY_DIR}/${QUERY_TYPE} --log=${RESULTS_DIR}/results_${QUERY_TYPE}_spark2.csv

#none
#./run_queries.sh --dbname=${DB_NAME} --iter=${ITER}  --engine=spark2 --dryRun=no --path=${PARENT_QUERY_DIR}/${QUERY_TYPE} --log=${RESULTS_DIR}/results_${QUERY_TYPE}_spark2_none.csv

#snappy
#./run_queries.sh --dbname=${DB_NAME} --iter=${ITER}  --engine=spark2 --dryRun=no --path=${PARENT_QUERY_DIR}/${QUERY_TYPE} --log=${RESULTS_DIR}/results_${QUERY_TYPE}_spark2_snappy.csv
